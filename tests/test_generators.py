...
class TestTransientLightcurveGenerator:
    ...
    def test_has_rise_and_decay(self, lc: Lightcurve) -> None:
        ...
        if len(before_peak) > 1:
            ...
            assert np.mean(before_peak[-len(before_peak) // 2 :]) > np.mean(
                before_peak[: len(before_peak) // 2]
            )

        if len(after_peak) > 1:
            # Allow for noise but general trend should be decreasing
            assert np.mean(after_peak[: len(after_peak) // 2]) > np.mean(
                after_peak[-len(after_peak) // 2 :]
            )
...
class TestLightcurveProperties:
    ...
    def test_normalization_properties(self, lc: Lightcurve) -> None:
        ...
        assert np.isclose(normalized.mean_flux, 0)

        # Check std is approximately 1 or 0 (no variation)
        assert np.isclose(normalized.std_flux, 1) or np.isclose(normalized.std_flux, 0)
    ...
    def test_normalization_scales_errors(self, lc: Lightcurve) -> None:
        ...
        if np.isclose(scale_factor, 0.0):
            assert all(np.isnan(x) for x in normalized.flux_err)
        else:
